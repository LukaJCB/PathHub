        ...
        <div className="bg-white rounded-lg shadow-md overflow-hidden">
          ...
          {post && (
            <div className="p-8 border-b border-gray-200">
              ...
              {post.gear && (
                <div className="flex items-center gap-2 text-sm text-gray-700 mb-4">
                  <span role="img" aria-label="gear">
                    {getGearEmoji(post.type)}
                  </span>
                  <span className="font-medium text-gray-800">{post.gear}</span>
                </div>
              )}
              {post.description && post.description.trim().length > 0 && (
                <p className="text-gray-700 mb-6 leading-relaxed whitespace-pre-line">{post.description}</p>
              )}
              <Link
              ...
              >
                {avatar ? (
                  <img src={avatar} alt="Avatar" className="w-9 h-9 rounded-full object-cover ring-2 ring-indigo-100" />
                ) : (
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                ...
                <div className="bg-purple-50 p-3 rounded-lg">
                  ...
                  <div className="text-lg font-bold text-purple-600">
                    {(post.metrics.duration / 3600000).toFixed(1)} h
                  </div>
                </div>
              ...
              <div className="space-y-4 mb-8">
                {comments.map((c) => (
                  <div key={uint8ToBase64Url(c.signature)} className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                    <Link to={`/user/${c.author}`} className="flex items-center gap-3 mb-2 hover:opacity-80">
