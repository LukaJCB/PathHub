...
import {
  maximumGainThreshold,
  minimumDistanceThreshold,
  minimumGainThreshold,
  parseFitData,
  parseGpxData,
  ParseResult,
  parseTcxData,
...
export function BulkImport() {
  ...
  async function processZip(file: File) {
    ...
    const result = Papa.parse<ActivityRecord>(text, {
      header: true,
    }).data.slice(0, 750)
    ...
    for (const [n, record] of result.entries()) {
      ...
      const [newGroup, newPage, newPostManifest, newManifest] = await createPost(
        content,
        {
          elevation: parsed.totalElevationGain,
        ...
        <div className="bg-white rounded-lg shadow-md p-8">
          ...
          <div className="mb-8">
            ...
            >
              ...
              <input type="file" accept=".zip" onChange={handleFile} className="hidden" id="zipInput" />
              <label
                htmlFor="zipInput"
              ...
              {progress === total && total > 0 && (
                <div className="bg-green-50 border border-green-200 rounded-lg p-4 text-center">
                  <Link to={`/user/${user.id}`}>
                    <p className="text-green-700 font-semibold">✓ Successfully imported {total} activities!</p>
                  </Link>
                </div>
              )}
            </div>
