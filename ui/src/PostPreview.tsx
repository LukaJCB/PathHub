  ...
  return (
    <div className="block hover:opacity-90 transition-opacity">
      <div className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
        {thumb && (
          <Link to={`/user/${userId}/${page}/${post.main[0]}`} aria-label="Open post" className="group relative block">
            <div className="aspect-video w-full overflow-hidden bg-gray-200">
              <img src={thumb} alt={post.title} className="w-full h-full object-cover" />
            </div>
            <span className="absolute bottom-2 right-2 px-2 py-1 text-xs bg-black/50 text-white rounded opacity-0 group-hover:opacity-100 transition-opacity">
              Open Post
            </span>
          </Link>
        )}
        ...
        <div className="p-4">
          <Link to={`/user/${userId}/${page}/${post.main[0]}`} aria-label="Open post" className="block">
          ...
          <Link to={`/user/${userId}`} aria-label="Open profile" className="hover:opacity-80">
            <div className="flex items-center gap-3 mb-3">
              ...
              <p className="text-xs text-gray-500">
                by <span className="font-medium text-gray-700">{username ?? userId}</span>
              </p>
            </div>
          </Link>
