  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      if (res.contentType === "image/svg+xml") {
        ...
        const blob = new Blob([bytesToArrayBuffer(res.body)], {
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <div className="max-w-2xl mx-auto bg-white rounded-lg shadow-md p-8">
        ...
        <p className="text-gray-600 mb-6">
          PNG, JPEG, or SVG. A square image looks best. Note this image will be stored unencrypted and publically
          available.
        </p>
          ...
          <div className="mb-6">
            <div className="flex items-center gap-6">
              <img
                src={previewUrl}
                alt="Preview"
                className="w-28 h-28 rounded-full object-cover ring-2 ring-indigo-100"
              />
              <button
                onClick={() => {
                  setPreviewUrl(null)
        ...
        <div className="flex gap-3">
          ...
          <button onClick={() => navigate(-1)} className="px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50">
            Cancel
          </button>
        </div>
      </div>
