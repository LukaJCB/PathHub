...
export const FollowerView: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const result = await getGroupStateForUser(
        user.manifest,
        user.masterKey,
        user.id,
        profileUserId,
      ...
      if (!result) {
        ...
        for (const f of followerIds) {
          ...
          const userInfo = await getUserInfo(
            userId,
            remoteStore.client,
            createAuthenticationClient("/auth"),
            user.token,
          )
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <div className="max-w-4xl mx-auto">
        <div className="bg-white rounded-lg shadow-md p-8">
          ...
          >
            {avatar ? (
              <img src={avatar} alt="Avatar" className="w-12 h-12 rounded-full object-cover ring-2 ring-indigo-100" />
            ) : (
